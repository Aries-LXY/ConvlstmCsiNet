...
def residual_network():
    ...
    def P3D_C(z):
        shortcut = z
        z_s = Conv3D(2, (1, 3, 3), padding='same', data_format="channels_last")(z)
        z1 = BatchNormalization()(z_s)

        z_t = add_common_layers(z_s)
        z_t = add([z, z_t])
        z2 = Conv3D(2, (3, 1, 1), padding='same', data_format="channels_last")(z_t)
        z2 = BatchNormalization()(z2)
        out = add([shortcut, z1, z2])
        out = LeakyReLU()(out)
        return out
...
for period in range(int(1500 / test_peroid)):
    ...
    path = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/TensorBoard_' + file + '.csv'
    ...
    # save
    # save and print loss
    filename = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/trainLoss_' + file + '.csv'
    ...
    filename = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/valLoss_' + file + '.csv'
    ...
    plt.figure()
    plt.plot(iters_train, train_Loss, 'g', label='ConvlstmCsiNet_C_trainloss')
    plt.plot(iters_train, val_Loss, 'k', label='ConvlstmCsiNet_C_valloss')
    plt.grid(True)
    plt.xlabel('epoch')
    plt.ylabel('loss')
    plt.legend(loc="upper right")
    picfile_loss = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/Loss_' + file + '.png'
    plt.savefig(picfile_loss)

    # serialize model to JSON
    model_json = autoencoder.to_json()
    outfile = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/model_' + file + '.json'
    with open(outfile, "w") as json_file:
        json_file.write(model_json)
    # serialize weights to HDF5
    outfile = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/model_' + file + '.h5'
    ...
    filename = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/rho' + file + '.csv'
    np.savetxt(filename, rho_all, delimiter=",")

    iters = range(0, len(rho_all) * test_peroid, test_peroid)
    plt.figure()
    plt.plot(iters, rho_all, 'g', label='ρ_C')
    plt.grid(True)
    plt.xlabel('epoch')
    plt.ylabel('ρ')
    plt.legend(loc="upper right")
    picfile_rho = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/rho_' + file + '.png'
    plt.savefig(picfile_rho)

    # print nmse
    nmse_all = np.append(nmse_all, minus_nmse)
    nmse_all = np.reshape(nmse_all, (-1,))
    filename = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/nmse_' + file + '.csv'
    np.savetxt(filename, nmse_all, delimiter=",")

    iters = range(0, len(nmse_all) * test_peroid, test_peroid)
    plt.figure()
    plt.plot(iters, nmse_all, 'g', label='10*log10(nmse)_C')
    plt.grid(True)
    plt.xlabel('epoch')
    plt.ylabel('nmse')
    plt.legend(loc="upper right")
    picfile_nmse = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/nmse_' + file + '.png'
    ...
    for i in range(n):
        ...
        ax.get_yaxis().set_visible(False)
        ax.invert_yaxis()
    picfile = 'CSI/result' + '/ConvlstmCsiNet_C_' + envir + '/dim_' + str(encoded_dim) + '/pic_' + file + '.png'
    plt.savefig(picfile)
